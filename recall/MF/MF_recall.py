...
class MFRecall(BaseRecall):
    ...
    def recall(self, userid, k=5):
        user_emb = np.array(self.user_emb[userid]).astype('float32')
        distances, indices = self.index.search(user_emb, k + 1)
        results = [(self.index2itemId[idx], dist) for idx, dist in zip(indices[0][1:], distances[0][1:])]
        return results
    
    
if __name__ == '__main__':
    mf_recall = MFRecall(
        '/Users/zhanghaoyang04/Desktop/Movie_Recsys/recall/MF/MF_emb/MF_user_emb.pkl',
        '/Users/zhanghaoyang04/Desktop/Movie_Recsys/recall/MF/MF_emb/MF_item_emb.pkl',
    )

    MFRecall.eval(val_data_path='/Users/zhanghaoyang04/Desktop/Movie_Recsys/cache/val_data.pkl', k=10)